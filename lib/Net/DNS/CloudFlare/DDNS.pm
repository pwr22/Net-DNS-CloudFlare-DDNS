...
# Clean up imports and stuff after we're done
use namespace::autoclean;

# VERSION

# Should the object produce logging output, basically
...
# See the POD at the end for the spec
...
sub _buildDomIds {
    ...
    for my $zone ( @{ $self->_config } ) {
        ...
        my $zoneMap;
        try { $zoneMap = $self->_getDomainIds($name) }
...
has '_domIds' => (
    ...
    clearer => '_clearDomIds',
    builder => '_buildDomIds',
    lazy    => 1
);
...
Readonly my @IP_URLS => map {"http://$_"}
    ( 'icanhazip.com', 'ifconfig.me/ip', 'curlmyip.com' );
...
sub update {
    ...
    carp "Cannot update records without an IP" and return
        unless Readonly my $ip => $self->_getIp;
        ...
        for my $dom ( @{ $zone->{domains} } ) {
            ...
            do {
                ...
                if ( $self->_lastIps->{ \$dom } eq $ip ) {
                    say "IP not changed for $dom, skipping update"
                        if $self->verbose;
                    next;
                }
            };
